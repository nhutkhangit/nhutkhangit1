...
{
	var b = document.getElementsByName('dx[]');
	if(tt == true)
	{
...
<form action="index.php?page=datxe" method="post" name="form" enctype="multipart/form-data">
...
<table cellpadding='1' cellspacing='1' width='100%'>
  <tr>
    <td colspan='12' align='center' class='title'><b>Danh sách đặt xe trực tuyến</b></td>
  </tr>
    ...
    <tr align=center>
      ...
      <td  class='title' >CMT/MST</td>
      <td  class='title' >Dòng xe</td>
			...
			if($n>0)
			{
				...
				foreach($mang as $row)
				{
                    ...
                    <tr align='center'<?php if($stt%2==0){?> class="trOdd"<?php }else{?> class="trEven"<?php }?> >
                      <td <?php if($stt%2==0){?> class="trOdd"<?php }else{?> class="trEven"<?php }?>><input type="checkbox" name="dx[]" id="dx[]" value="<?php echo $row[0];?>" /></td>
                      ...
                      <td align="center" <?php if($stt%2==0){?> class="trOdd"<?php }else{?> class="trEven"<?php }?>><?php echo $dx->tensp($row[6]);?></td>
