...
<form action="index.php?page=logo" method="post" name="form">
<table width='100%' align='center' cellpadding='2' cellspacing='0' style="margin:5px;">
  <tr>
    <td width="100%">
        
    <div class="buttons">
    <button type="button" class="positive" name="save" onclick="window.location='index.php?page=logo&act=them'">
    ...
    <button type="submit" class="positive" name="delete">
     ...
     <td height="42"><b>Trạng thái </b>&nbsp;&nbsp;
       <label>
         <select name="select2" id="select2" onchange="if(this.value==''){window.location='index.php?page=logo';}else{window.location='index.php?page=logo&xemtrangthai='+this.value;}">
...
<table cellpadding='1' cellspacing='1' width='100%'>
  <tr>
    <td colspan='12' align='center' class='title'><b>Danh sách logo show</b></td>
  </tr>
			...
			if($n>0)
			{
				...
				foreach($mang as $row)
				{
                    ...
                    <tr align='center'<?php if($stt%2==0){?> class="trOdd"<?php }else{?> class="trEven"<?php }?> >
                      ...
                      <td <?php if($stt%2==0){?> class="trOdd"<?php }else{?> class="trEven"<?php }?>><a href="#" id="" onmouseout="hidetip();" onmouseover="showtip('<?php echo $row[2];?>');"><img src="../data/logo/<?php echo $row[1];?>" width="130" height="60" /></a></td>
                      ...
                      <td <?php if($stt%2==0){?> class="trOdd"<?php }else{?> class="trEven"<?php }?>><label>
                         <input type="checkbox" name="checkbox2" id="checkbox2"<?php if($row[4]==1){?> checked="checked"<?php }?> onclick="if(this.checked==true){ window.location='index.php?page=logo&id=<?php echo $row[0];?>&trangthai=1';}else{window.location='index.php?page=logo&id=<?php echo $row[0];?>&trangthai=0';}" />
                      </label></td>
                      <td <?php if($stt%2==0){?> class="trOdd"<?php }else{?> class="trEven"<?php }?>><a href="index.php?page=logo&act=sua&id=<?php echo $row[0];?>"><img src="images/edit.png" /></a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="index.php?page=logo&act=xoa&id=<?php echo $row[0];?>" onclick="return confirm('Bạn có chắc chắn muốn xóa.');"><img src="images/delete.png" /></a></td>
                    </tr>
                    <?php
				}
			}
