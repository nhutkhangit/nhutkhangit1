<?php
		...
		if(isset($_GET['act']) and $_GET['act']=='them')//Them moi 
		{
			...
			if(isset($_POST['them']))
			{
				...
				if($_FILES['file']['type']=='image/jpeg' || $_FILES['file']['type']=='image/gif' || $_FILES['file']['type']=='image/png')
				{
					...
					{
							move_uploaded_file($_FILES['file']['tmp_name'],'../data/slide/'.$_FILES['file']['name']);
							$sl->hinhanh = $_FILES['file']['name'];
							if($sl->them())
							{
								?>
                                <script language="javascript">
								alert('Thêm slide show thành công.');
							...
							else
							{
								?>
                                <script language="javascript">
								alert('Thêm slide show không thành công.');
								</script>
                                <?php
							}
					}
		...
		if(!isset($_GET['act']))//Quan ly 
		{
			if(isset($_GET['xemtrangthai']) and $_GET['xemtrangthai']!='')
			{
				...
				$count = mysql_num_rows(mysql_query("SELECT * from slide where status='$trangthai'"));
				$page = findpages($count,$limit);
				$sql = "SELECT * from slide where status='$trangthai' order by orders ASC limit $start,$limit";
				$mang = $sl->quanly($sql);
				$chuoi ='index.php?page=slide&xemtrangthai='.$trangthai;
			}
			else
			{
				$limit =10;
				$start =  findstart($limit);
				$count = mysql_num_rows(mysql_query("SELECT * from slide"));
				$page = findpages($count,$limit);
				$sql = "SELECT * from slide order by orders ASC limit $start,$limit";
				$mang = $sl->quanly($sql);
				$chuoi ='index.php?page=slide';
			}
			...
			if(isset($_POST['capnhap']))//cap nhap thu tu
			{
				if(isset($_POST['thutu']))
				{
					foreach($_POST['thutu'] as $k=>$v)
					{
						if($v>0 && is_numeric($v))
						{
							mysql_query("update slide set thutu='$v' where id='$k'");
						}
					}
					?>
					<script language="javascript">
			...
			if(isset($_POST['capnhap']))
			{
				if($_FILES['filesua']['name']=="")
				{
					...
					if($sl->sua())
					{
						?>
						<script language="javascript">
						alert('Sửa slide show  thành công.');
						window.location='index.php?page=slide';
						</script>
						<?php
					}
					else
					{
						?>
						<script language="javascript">
						alert('Sửa slide show không thành công.');
						</script>
						<?php
					}
				}
				else
				{
					if( $_FILES['filesua']['type']=='image/jpeg' || $_FILES['filesua']['type']=='image/gif' || $_FILES['filesua']['type']=='image/png')
					{
							...
							{
								move_uploaded_file($_FILES['filesua']['tmp_name'],'../data/slide/'.$_FILES['filesua']['name']);
								$sl->id = $mang[0];
