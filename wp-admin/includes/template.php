 ...
 * @param int $descendants_and_self Optional. ID of the category to output along with its
 *     descendants. Default 0.
...
function wp_terms_checklist($post_id = 0, $args = []){
	...
	if (is_array($r['selected_cats'])){
		...
		$args['selected_cats'] = wp_get_object_terms($post_id, $taxonomy,
...
$terms = get_terms($taxonomy,
	['orderby' => 'count', 'order' => 'DESC', 'number' => $number, 'hierarchical' => FALSE]);
	...
	foreach ($taxonomy_names as $taxonomy_name){
		...
		if ($taxonomy->hierarchical && $taxonomy->show_ui){
			...
			if (FALSE === $terms){
				...
				wp_cache_add($post->ID, wp_list_pluck($terms, 'term_id'),
...
$content = apply_filters('wp_comment_reply', '',
	['position' => $position, 'checkbox' => $checkbox, 'mode' => $mode]);
                ...
                <div id="com-reply" style="display:none;">
                    <div id="replyrow" style="display:none;">
                        ...
                        <fieldset class="comment-reply">
							...
							<?php
							wp_nonce_field('replyto-comment', '_ajax_nonce-replyto-comment',
								FALSE);
							if (current_user_can('unfiltered_html')){
								wp_nonce_field('unfiltered-html-comment',
									'_wp_unfiltered_html_comment', FALSE);
							}
							?>
                        </fieldset>
						<?php if ($table_row) : ?>
...
<div class="hidden" id="trash-undo-holder">
    <div class="trash-undo-inside"><?php printf(__('Comment by %s moved to the trash.'),
		'<strong></strong>'); ?>
        <span class="undo untrash"><a href="#"><?php _e('Undo'); ?></a></span></div>
</div>
<div class="hidden" id="spam-undo-holder">
    <div class="spam-undo-inside"><?php printf(__('Comment by %s marked as spam.'),
		'<strong></strong>'); ?>
...
function _list_meta_row($entry, &$count){
	...
	$r .= "\n\t\t<div class='submit'>";
	$r .= get_submit_button(__('Delete'), 'deletemeta small', "deletemeta[{$entry['meta_id']}]",
		FALSE,
		['data-wp-lists' => "delete:the-list:meta-{$entry['meta_id']}::_ajax_nonce=$delete_nonce"]);
	$r .= "\n\t\t";
	$r .= get_submit_button(__('Update'), 'updatemeta small', "meta-{$entry['meta_id']}-submit",
		FALSE,
...
<table id="newmeta">
    <thead>
    <tr>
        <th class="left"><label for="<?php echo $meta_key_input_id; ?>"><?php _ex('Name',
			'meta name') ?></label></th>
            ...
            <select id="metakeyselect" name="metakeyselect">
				...
				foreach ($keys as $key){
					if (is_protected_meta($key,
							'post') || !current_user_can('add_post_meta', $post->ID, $key)){
						continue;
					}
        ...
        <td colspan="2">
            <div class="submit">
				<?php submit_button(__('Add Custom Field'), '', 'addmeta', FALSE,
					['id' => 'newmeta-submit', 'data-wp-lists' => 'add:the-list:newmeta']); ?>
            </div>
...
if ($for_post){
	$edit = !(in_array($post->post_status,
			['draft', 'pending']) && (!$post->post_date_gmt || '0000-00-00 00:00:00' == $post->post_date_gmt));
}
...
function parent_dropdown($default = 0, $parent = 0, $level = 0, $post = NULL){
	global $wpdb;
	$post  = get_post($post);
	$items = $wpdb->get_results($wpdb->prepare("SELECT ID, post_parent, post_title FROM $wpdb->posts WHERE post_parent = %d AND post_type = 'page' ORDER BY menu_order",
		$parent));
...
?>
<form enctype="multipart/form-data" id="import-upload-form" method="post" class="wp-upload-form" action="<?php echo esc_url(wp_nonce_url($action,
	'import-upload')); ?>">
    <p>
        <label for="upload"><?php _e('Choose a file from your computer:'); ?></label> (<?php printf(__('Maximum size: %s'),
		$size); ?>)
 ...
 *                                              should display. Available contexts vary from screen
 *     to screen. Post edit screen contexts include 'normal', 'side', and 'advanced'. Comments
 *     screen contexts include 'normal' and 'side'. Menus meta boxes (accordion sections) all use
 *     the 'side' context. Global default is 'advanced'.
 * @param string $priority Optional. The priority within the context where the boxes
 *                                              should show ('high', 'low'). Default 'default'.
 * @param array $callback_args Optional. Data that should be set as the $args property
 *                                              of the box array (which is the second parameter
 *     passed to your callback). Default null.
 */
function add_meta_box(
	...
	if (empty($screen)){
		...
		foreach ($screen as $single_screen){
			add_meta_box($id, $title, $callback, $single_screen, $context, $priority,
				$callback_args);
		}
	}
			...
			if (isset($wp_meta_boxes[$page][$context][$priority])){
				foreach ((array) $wp_meta_boxes[$page][$context][$priority] as $box){
					...
					if ('dashboard_browser_nag' != $box['id']){
						...
						echo '<span class="screen-reader-text">' . sprintf(__('Toggle panel: %s'),
								$widget_title) . '</span>';
 ...
 *
 * @param string $id Meta box ID (used in the 'id' attribute for the meta box).
 * @param string|array|WP_Screen $screen The screen or screens on which the meta box is shown (such
 *     as a post type, 'link', or 'comment'). Accepts a single screen ID, WP_Screen object, or
 *     array of screen IDs.
 * @param string $context The context within the screen where the box is set to display.
 *                                        Contexts vary from screen to screen. Post edit screen
 *     contexts include 'normal', 'side', and 'advanced'. Comments screen contexts include 'normal'
 *     and 'side'. Menus meta boxes (accordion sections) all use the 'side' context.
 */
 ...
 * @param callable $callback Function that echos out any content at the top of the section (between
 *     heading and fields).
 * @param string $page The slug-name of the settings page on which to show the section. Built-in
 *     pages include
 *                           'general', 'reading', 'writing', 'discussion', 'media', etc. Create
 ...
 * @param string $code Slug-name to identify the error. Used as part of 'id' attribute in HTML
 *     output.
 * @param string $message The formatted message text to display to the user (will be shown inside
 *     styled
 *                        `<div>` and `<p>` tags).
 ...
 * hasn't submitted data (i.e. when they first load an options page, or in the {@see
 * 'admin_notices'} action hook).
 *
 * @since 3.0.0
 *
 * @global array $wp_settings_errors Storage array of errors registered during this pageload
...
function get_settings_errors($setting = '', $sanitize = FALSE){
	...
	if (isset($_GET['settings-updated']) && $_GET['settings-updated'] && get_transient('settings_errors')){
		$wp_settings_errors = array_merge((array) $wp_settings_errors,
 ...
 *
 * The password is passed through esc_attr() to ensure that it is safe for placing in an html
 * attribute.
 *
 * @since 2.7.0
 */
...
?>
<title><?php bloginfo('name') ?> &rsaquo; <?php echo $title ?> &#8212; <?php _e('WordPress'); ?></title>    <?php
 ...
 *                                       include 'primary', 'small', and 'large'. Default
 *     'primary'.
 * @param string $name The HTML name of the submit button. Defaults to "submit". If no
 *                                       id attribute is given in $other_attributes below, $name
 *     will be used as the button's id.
 * @param bool $wrap True if the output button should be wrapped in a paragraph tag,
 *                                       false otherwise. Defaults to true
 * @param array|string $other_attributes Other attributes that should be output with the button,
 *     mapping attributes to their values, such as setting tabindex to 1, etc. These key/value
 *     attribute pairs will be output as attribute="value", where attribute is the key. Other
 *     attributes can also be provided as a string such as 'tabindex="1"', though the array format
 *     is preferred. Default null.
 */
 ...
 *                                       If no id attribute is given in $other_attributes below,
 ...
 * @param array|string $other_attributes Optional. Other attributes that should be output with the
 *     button, mapping attributes to their values, such as `array( 'tabindex' => '1' )`. These
 *     attributes will be output as `attribute="value"`, such as
 *                                       `tabindex="1"`. Other attributes can also be provided as a
 *     string such as `tabindex="1"`, though the array format is typically cleaner. Default empty.
 *
...
?> <?php language_attributes(); ?>><![endif]--><!--[if !(IE 8) ]><!-->
...
<head>
	 ...
	 * @since 3.0.0
	 *
	 * @param string $hook_name The hook name (also known as the hook suffix) used to determine the
	 *     screen.
	 *
	...
	function convert_to_screen($hook_name){
		if (!class_exists('WP_Screen')){
			_doing_it_wrong('convert_to_screen(), add_meta_box()',
				__("Likely direct inclusion of wp-admin/includes/template.php in order to use add_meta_box(). This is very wrong. Hook the add_meta_box() call into the add_meta_boxes action instead."),
				'3.3.0');
